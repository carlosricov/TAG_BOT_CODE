...
def Therapy_Mode(image, hsv, boundary_corners, last_send_time, therapy_zone, port, last_sent_mag, max_magnitude, output_file, game_cntr, game_start_time):
    ...
    if(last_sent_mag == 1):
        ...
        if(RECIEVING):
            ...
            if(hit_var == ''):
                delay = 0
            elif(int(str(hit_var)) == 1):
                while(str(port.readline().decode()) != ''):
                    port.readline()
                # if you were hit, add 1 to zone
                therapy_zone += 1
                OutputHits.Write_To_Output_File(output_file, "TherapyMode", game_cntr, time.time() - game_start_time, therapy_zone)
        else:
            therapy_zone += 1
            OutputHits.Write_To_Output_File(output_file, "TherapyMode", game_cntr, game_start_time - time.time(), therapy_zone)
...
def Time_Trial(image, hsv, boundary_corners, last_send_time, port, last_sent_mag, game_score, max_magnitude, output_file, game_cntr, game_start_time):
    ...
    if(RECIEVING):
        ...
        if(hit_var == ''):
            ...
            game_score += 1
            OutputHits.Write_To_Output_File(output_file, "TimeTrial", game_cntr, time.time() - game_start_time, game_score)
...
def Whack_a_Mole(image, hsv, boundary_corners, last_send_time, port, last_sent_mag, game_score, max_magnitude, output_file, game_cntr, game_start_time):
    ...
    if(RECIEVING):
        ...
        if(hit_var == ''):
            ...
            print("Score : " + str(game_score))
            game_score += 1
            OutputHits.Write_To_Output_File(output_file, "WhackAMole", game_cntr, time.time() - game_start_time, game_score)
